...
<%--            <input type="checkbox" class="form-control" id="inputRole" value="2" name="role">--%>
            <c:forEach items="${roles}" varStatus="item">
                <label for="inputRole${item.index}">${item.current.roleName}</label>
                <input type="checkbox" class="form-control" id="inputRole${item.index}" value="${item.current.idRole}" name="role">
            </c:forEach>
        </div>
